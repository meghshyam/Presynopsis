...
effective collaboration among them can happen. E.g., if there are two
quadcopters covering a large plane, one quadcopter need to identify that there
is another quadcopter which will cover the left side and it need to cover the
right side.
...
quadcopter as shown in Figure~\ref{fig:fiducialQuadcopter}(a). 
...
Figure~\ref{fig:fiducialQuadcopter}(b),(c) and (d) shows the detection of
and classification fiducials from different angles. As each fiducial carry a
