...
\begin{figure}
  ...
  \caption{ \label{fig:teaser} The long range photograph of a scene
...
The goal in this paper is to create panoramic images of scenes using a
...
image contains vacant space.  In this paper, we use IMU information
...
%{\bf Limitations} 
In this paper, we assume that the scene lies on a planar surface, or
...
features and fail when images have gaps. In this paper, we discuss how
...
is beyond the scope of this paper.  \emph{Note that trying to create a
...
\begin{figure}[t!]
  ...
  \caption{ \label{fig:selection} We align the image stream with IMU
    data, and then transform the video into a set of interesting
...
\caption{Candidate images for a super-panorama taken from different
  depths. For context, see Fig.~\ref{fig:results}.  These images are
...
In this paper, we have described a method of imaging large scenes
